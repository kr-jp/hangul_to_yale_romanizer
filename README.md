...
- 4種類の出力形式（Plain / Gloss / LaTeX / Markdown）と例文番号付与
...
2. **音節境界を明示** — `.` や `-` を入力して区切ると、確率モデルを介さず決定論的に変換します（例: `ce.nel` → 저널）。
...
### Plain
素のテキスト出力。コピー時は半角空白がタブに置換されます（Wordでのグロス作成用）。「例文番号」スイッチをONにすると各行の先頭に `(1) (2) (3)…` が付きます。
...
### URL共有
「URL共有」ボタンを押すと、現在の入力テキスト・全オプション（変換方向・両唇音・区切り文字・併記・出力形式・例文番号）・単語クリックで切り替えた候補選択をURLにエンコードしてクリップボードにコピーします。共同研究者にこのURLを送れば、同じ画面が再現されます。

URLパラメータがある場合は、ブラウザの`localStorage`設定よりも優先して適用されます。

### バックアップ
「バックアップ」ボタンで、入力・出力・全オプション・候補選択をひとつのJSONファイルにまとめて保存できます。後から内容を復元したり、別環境への引き継ぎに利用できます。
